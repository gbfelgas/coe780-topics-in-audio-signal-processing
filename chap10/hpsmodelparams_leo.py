...
from hpsmodelparams_danilo import hpsmodelparams
...
def hpsmodelparams_shapeinvariance(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timemapping=None,Ns=1024,H=256):
    '''
...
def hpsmodelparams_harmfiltering(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timemapping=None,Ns=1024,H=256):
    ...
    while pout<poutend:
        ...
        #------------ 10.25 CODE -----------#
        yhloc[1::2] = 0 # set to zero the frequency of even harmonics so that they won't be synthesized
        #------------ 10.25 CODE -----------#
...
def hpsmodelparams_pitchdisc(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timemapping=None,Ns=1024,H=256):
    ...
    while pout<poutend:
        ...
        if (f0>0): # it has harmonics
            nst = np.round(12*np.log2(f0/55))     # closest semitone
...
if __name__=='__main__':
    ...
    x, fs = af.read('audio\\tedeum2.wav')       # input signal (x) and sampling rate (fs)
    w = sig.windows.hann(1025,False)            # analysis window (odd size)
    N = 1024;                                   # FFT size (minimum 512)
    t = -100;                                   # threshold in negative dB 
    nH = 50;                                    # maximum number of harmonics
    minf0 = 100;                                # minimum f0 frequency in Hz, 
    maxf0 = 400;                                # maximim f0 frequency in Hz, 
    f0et = 5;                                   # error threshold in the f0 detection
    maxhd = 1.5;                                # max. relative deviation in harmonic detection
    stocf = 10;                                 # decimation factor of mag spectrum for stochastic analysis
    Ns = 1024;                                  # FFT size for synthesis
    H = 256;                                    # hop size for analysis and synthesis

    #timeMapping = np.array([[0,(len(x)-1)/fs],[0,(len(x)*1.5-1)/fs]])                  # Time-scaling: np.array([[input time],[output time]])
    timeMapping = None                                                                  # No time-scaling

    y, yh, ys= hpsmodelparams(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timeMapping,Ns,H)
    af.write('audio\\tedeum2_resynth.wav',y,fs)
    print('Done base-resynth.')

    y, yh, ys= hpsmodelparams_shapeinvariance(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timeMapping,Ns,H)
    af.write('audio\\tedeum2_shapeinvariance.wav',y,fs)
    print('Done shape invariance.')

    y, yh, ys= hpsmodelparams_harmfiltering(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timeMapping,Ns,H)
    af.write('audio\\tedeum2_harmfiltering.wav',y,fs)
    print('Done harmonic filtering.')

    y, yh, ys= hpsmodelparams_pitchdisc(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf,timeMapping,Ns,H)
    af.write('audio\\tedeum2_pitchdisc.wav',y,fs)
    print('Done pitch discretization.')
