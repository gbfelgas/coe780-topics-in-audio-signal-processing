...
def hpsmodelparams_timbre_gabi(
    x: np.ndarray,
    fs: float,
    w: np.ndarray,
    N: int,
    ...
    Args:
        ...
        fscale (int): Harmoni frequency scal factor
        timbremapping (np.ndarray): Mapping of harmonic frequencies
    ...
    while pout < poutend:
        ...
        # transformations:
        # transposition and timbre scale
        ...
        if (f0 > 0):
            thloc = np.interp(
                ...
                timbremapping[1, :],
                timbremapping[0, :],
            ) / fs*Ns                                                               # mapped harmoni frequencies

            idx = np.nonzero((hloc > 0) * (hloc < Ns*.5))[0]                        # harmonic indexes in frequency range
            yhmag = np.interp(
                thloc,
                np.append(np.insert(hloc[idx], 0, 0), Ns),
                np.append(np.insert(hmag[idx], 0, hmag[0]), hmag[-1])
        ...
        # mapped coef. indexes
        trescoef = np.interp(
            np.minimum(fs/2, np.arange(len(mYsenv))/(len(mYsenv) - 1) * frescoef),
            timemapping[1, :],
            timemapping[0, :]
        )
        # interpolated envelope                                   
        mYsenv = np.interp(
            trescoef/frescoef * (len(mYsenv) - 1),
            np.arange(len(mYsenv)),
            mYsenv
        )
