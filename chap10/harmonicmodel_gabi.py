...
def harmonicmodel(
    ...
    while pin < pend:
        ...
        if f0detection == 'twm':
            raise Exception("Ainda não foi implementado...")
        # callyin.py
        if f0detection == 'yin':
        ...
        while (f0 > 0) and (hi < nH) and (hf[hi] < fs/2):
            peak = np.abs((ploc[:npeaks] - 1)/N*fs - hf[hi])                                # find harmonic peaks
            dev = np.min(peak)                                                              # closest peak
            pei = np.argmin(peak)

            if (hi==1) and not (hloc[:hi] == ploc[pei]).any() and (dev < maxhd * hf[hi]):
                hloc[hi] = ploc[pei]                                                        # harmonic locations
                hmag[hi] = pmag[pei]                                                        # harmonic magnitudes
                hphase[hi] = pphase[pei]                                                    # harmonic phases
            hi += 1                                                                         # increase harmonic index

        hloc[:hi] = (hloc[:hi] != 0) * ((hloc[:hi] - 1)*Ns/N)                               # synthesis locs
