...
def spsmodel(x,fs,w,N,t,maxnS,stocf,Ns=1024,H=256,effect=None,Filter=None,fshift=None,fstretch=None,fscale=None,tscale=None):
    '''
    ...
    #Time scaling code 10.19
    if effect == 'timescale': 
        if tscale is 1:
            timemapping_inp = np.array([0, 1]);                        # input time (sec)
            timemapping_out = np.array([0, 0.5]);                        # output time (sec)
            plt.stem(timemapping_out, timemapping_inp)
            plt.xlabel(r"Output time (sec)")
            plt.ylabel(r"Input time (sec)",)
            plt.title(r"Time mapping");
            plt.grid(True, color = '0.7', linestyle='-', which='major', axis='both')
            plt.grid(True, color = '0.9', linestyle='-', which='minor', axis='both')
            plt.tight_layout()
            plt.savefig("C:/Users/frota/Documents/mestrado/Top_esp_Audio/figures/tscale1.jpg", dpi=600, bbox_inches='tight')
        elif tscale is 2:
            timemapping_inp = np.array([0, 1]);                        # input time (sec)
            timemapping_out = np.array([0, 2]);                        # output time (sec)
            plt.stem(timemapping_out, timemapping_inp)
            plt.xlabel(r"Output time (sec)")
            plt.ylabel(r"Input time (sec)",)
            plt.title(r"Time mapping");
            ...
            plt.grid(True, color = '0.9', linestyle='-', which='minor', axis='both')
            plt.tight_layout()
            plt.savefig("C:/Users/frota/Documents/mestrado/Top_esp_Audio/figures/tscale2.jpg", dpi=600, bbox_inches='tight')
        elif tscale is 3:  #Time scaling code 10.22
            tm = np.arange(0.01,0.94,0.01);
            timemapping_inp = np.concatenate(([0],tm+0.05*np.sin(8.6*np.pi*tm), [1]));  # input time (sec)
            timemapping_out = np.concatenate(([0],tm,[1]));                             # output time (sec)
            plt.plot(timemapping_out, timemapping_inp)
            plt.xlabel(r"Output time (sec)")
            plt.ylabel(r"Input time (sec)",)
            plt.title(r"Time mapping");
            plt.grid(True, color = '0.7', linestyle='-', which='major', axis='both')
            plt.grid(True, color = '0.9', linestyle='-', which='minor', axis='both')
            plt.tight_layout()
            plt.savefig("C:/Users/frota/Documents/mestrado/Top_esp_Audio/figures/tscale3.jpg", dpi=600, bbox_inches='tight')
        timemapping = np.vstack((timemapping_inp,timemapping_out))
        print(timemapping)
        timemapping = timemapping*soundlength/fs;
        outsoundlength = 1+np.round(timemapping[-1,-1]*fs, decimals=0); #length of output sound
        outsoundlength = outsoundlength.astype(int)
    ...
    #Time scaling code 10.20
    if effect == 'timescale':
        yh = np.zeros(outsoundlength+Ns//2);         # output sine component
        ys = np.zeros(outsoundlength+Ns//2);         # output residual componentw = w/sum(w);  
    else:
    ...
    #Time scaling code 10.21
    if effect == 'timescale':
        poutend = outsoundlength-hNs; # last sample to start a frame
        pout = pin;
        minpin = 1+np.maximum(hNs,hM);
        maxpin = np.min(len(x)-np.maximum(hNs,hM)-1);

        while pout<poutend:
            #pin = np.round( np.interp(timemapping[1,:],timemapping[0,:],pout/fs,'linear','extrap') * fs );
            pin = np.round( np.interp(pout/fs,timemapping[1,:],timemapping[0,:]) * fs );
            pin = np.maximum(minpin,pin);
            pin = np.minimum(maxpin,pin);
            pin = pin.astype(int);
            #-----analysis-----
            
            #Up to here: Time scaling code 10.21
            ...
            ys[ro:ro+Ns]+= ysw * sws;                        # overlap-add for stochasti
...
if __name__=='__main__':
    ...
    #time scaling
    tscale = 1 #1: Time compression; 2: Time expansion; 3: sine effect
    y, yh, ys = spsmodel(x,fs,w,N,t,maxnS,stocf,effect='timescale', tscale=tscale)

    if tscale is 1:
        auxName = inputFile.split('.wav')[0]
        #auxName = auxName+'_tscale_'+str(round(tscale,2))+'_t'+str(t)+'_nS'+str(maxnS)
        auxName = auxName+'_tscale_'+'_t_fast'+str(t)+'_nS'+str(maxnS)
        af.write(auxName+'_hps.wav',y,fs)
        af.write(auxName+'_h.wav',yh,fs)
        af.write(auxName+'_s.wav',ys,fs)
    elif tscale is 2:
        auxName = inputFile.split('.wav')[0]
        #auxName = auxName+'_tscale_'+str(round(tscale,2))+'_t'+str(t)+'_nS'+str(maxnS)
        auxName = auxName+'_tscale_'+'_t_slow'+str(t)+'_nS'+str(maxnS)
        af.write(auxName+'_hps.wav',y,fs)
        af.write(auxName+'_h.wav',yh,fs)
        af.write(auxName+'_s.wav',ys,fs)
    elif tscale is 3:
        auxName = inputFile.split('.wav')[0]
        #auxName = auxName+'_tscale_'+str(round(tscale,2))+'_t'+str(t)+'_nS'+str(maxnS)
        auxName = auxName+'_tscale_'+'_t_funny'+str(t)+'_nS'+str(maxnS)
        af.write(auxName+'_hps.wav',y,fs)
        af.write(auxName+'_h.wav',yh,fs)
        af.write(auxName+'_s.wav',ys,fs)
