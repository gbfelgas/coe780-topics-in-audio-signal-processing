...
def hpsmodelparams_timbre(x, fs, w, N, t, nH, minf0, maxf0, f0et, maxhd, stocf, timemapping = None):
    ...
    while pout < poutend:
        pinaux = np.interp(timemapping[1,:], timemapping[0,:], np.array([0, pout/fs]))
        pin = np.round(pinaux[1]*fs)
        pin = max(minpin,pin)
        pin = min(maxpin,pin)
        pin = int(pin)
        #-----analysis-----#
        ...
        if (f0>0):
            ...
            idx = np.where(hloc>0 and hloc<Ns*5)         #harmonic indexes in frequency range
...
# # Authors: J. Bonada, X. Serra, X. Amatriain, A. Loscos
# #-----male to female-----
# #inputFile='basket.wav'
# x, fs = sf.read("C:/Users/frota/Documents/mestrado/Top_esp_Audio/audios/basket.wav")
# #x, fs = af.read(inputFile);
# w = sig.windows.blackmanharris(1025)
# fscale = 2
...
# #-          hpsmodelparams(x, fs, w, N, t, nH, minf0, maxf0, f0et, maxhd, stocf, timemapping = None, fscale, timbremapping) 
# y, yh, ys = hpsmodelparams_timbre(x, fs, w, 2048, -150, 200, 100, 400, 1, 0.2, 10, timemapping = None)
# sf.write('./output.wav', 1.5*y, fs)
