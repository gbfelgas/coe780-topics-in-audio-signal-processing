...
def hpsmodel(x,fs,w,N,t,nH,minf0,maxf0,f0et,maxhd,stocf):
    ...
    #-----analysis-----#
        ...
        mYr = abs(Yr[:Ns//2+1]);               # magnitude spectrum of residual
        mYsenv = sig.decimate(mYr,stocf);          # decimate the magnitude spectrum
        #-----synthesis-----#
        mYs = interp(mYsenv,stocf);                # interpolate to original size
        roffset = int(np.ceil(stocf/2)-1);                 # interpolated array offset
        mYs = np.concatenate((mYs[0]*np.ones(roffset), mYs[:Ns//2+1-roffset]));
        pYs = 2*np.pi*np.random.uniform(0,1,(Ns//2+1));                 # generate phase random values
        mYs1 = np.concatenate((mYs[:Ns//2+1], mYs[::-1]));    # create magnitude spectrum
        pYs1 = np.concatenate((pYs[:Ns//2+1], -1*pYs[::-1])); # create phase spectrum
        ...
        ysw = fftshift(np.real(ifft(Ys)));            # stoc. in time domain using IFFT
        yh[ri:ri+Ns] = yh[ri:ri+Ns]+yhw[:Ns]*sw;   # overlap-add for sines
        ys[ri:ri+Ns] = ys[ri:ri+Ns]+ysw[:Ns]*sws;  # overlap-add for stoch.
        pin = pin+H;                                     # advance the sound pointer
    y= yh+ys; # sum sines and stochastic
    return y, yh,ys, f0s, plocs
