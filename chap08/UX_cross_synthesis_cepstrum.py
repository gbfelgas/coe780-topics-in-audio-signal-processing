# UX_cross_synthesis_cepstrum.py   [DAFXbook, 2nd ed., chapter 8]
...
#---- computing cepstrum ----
  flog = np.log(0.00001+np.abs(f_env))
  cep = np.fft.ifft(flog)  # cepstrum of sound 2

#---- liftering cepstrum ----
  cep_cut = np.zeros(s_win)
  cep_cut[0:order_sou] = cep[0:order_sou]
  cep_cut[0] = cep_cut[0]/2
  flog_cut = 2*np.real(np.fft.fft(cep_cut))

#---- computing spectral enveloppe ----
  f_env_out = np.exp(flog_cut)  # spectral shape of sound 2 
...
sf.write("C:/Users/frota/Documents/mestrado/Top_esp_Audio/audios/test_UX_cross_syn_cepstrum.wav", DAFx_out_norm, fs_1)
...
plt.legend()
plt.savefig("C:/Users/frota/Documents/mestrado/Top_esp_Audio/figures/test_UX_cross.jpg", dpi=600, bbox_inches='tight')
